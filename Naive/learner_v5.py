...
class LIFNeuron(nn.Module):
    ...
    def __init__(self, tau=0.5, vth=0.5):
        super().__init__()
        self.tau = tau  # membrane time constant
        self.vth = vth  # spike threshold - LOWERED for easier firing
...
class SNNTextModel(nn.Module):
    ...
    def encode_to_spikes(self, tokens):
        ...
        # Key: make spike probability high enough that we get actual spikes
        spike_probs = torch.sigmoid(embedded * 2.0)  # Scale up to get higher probabilities
        
        # Create spike train
        spikes = []
        for t in range(self.num_steps):
            spike_t = torch.bernoulli(spike_probs)
