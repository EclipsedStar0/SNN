...
class SNNTextModel(nn.Module):
    ...
    def encode_to_spikes(self, tokens):
        ...
        # Use deterministic spike generation based on embedding magnitude
        # This reduces stochastic noise during training
        spike_probs = torch.sigmoid(embedded)  # Map to [0,1]
        
        # Create spike train: higher prob = more consistent spikes
        # Instead of random sampling, use threshold-based conversion
        spikes_list = []
        for t in range(self.num_steps):
            # Add temporal jitter to spike times
            threshold = (t + 1) / self.num_steps
            spike_t = (spike_probs > threshold).float()
            spikes_list.append(spike_t.unsqueeze(-1))
...
def train_epoch(model, train_loader, optimizer, device):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # Gradient clipping (important for SNNs to prevent exploding gradients)
...
if __name__ == "__main__":
    ...
    print(f"New Vocabulary: {list(token_to_id.keys())}")
