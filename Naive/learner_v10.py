...
class SNNTextModel(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, prompt_tokens, id_to_token, max_length=50, temperature=1.0):
        ...
        for _ in range(max_length):
            ...
            if next_token == token_to_id.get("<EOS>"):
                print("STOPPING ON END_OF_STREAM")
                break
            else:
                print(next_token, " != ", token_to_id.get("<EOS>"))
...
if __name__ == "__main__":
    ...
    # Create character vocabulary from sample text
    small_input = ["The quick brown fox jumps over the lazy dog.",
    "Hello world and welcome to machine learning.",
    "Spiking neural networks are bio-inspired models.",
    "This model learns character level patterns in text.",
    ...
    "As the simulation progresses, the synaptic weights are adjusted based on the STDP rule, potentially making it more likely for the pattern to be detected if the network learns to recognize it.",
    "The leaky integrate-and-fire model with refractory periods and decay factors can cause neurons to spike at specific intervals, contributing to the pattern detection.",
    "SNNs excel at processing temporal information, as they naturally encode and process time through the timing of spikes. This capability is crucial for applications like speech recognition, time-series prediction, and dynamic sensory processing.",
    "raining SNNs is more challenging compared to traditional ANNs due to the discrete and non-differentiable nature of spikes. Researchers have developed various approaches, such as converting trained ANNs to SNNs and using surrogate gradient methods to overcome this hurdle.",
    "SNNs' ability to process sensory information in real-time makes them ideal for robotic applications, including autonomous navigation, sensorimotor control, and adaptive behavior.",
    ...
    "While the analysis provides comprehensive insights into neuron models, training paradigms, and performance metrics, several limitations must be acknowledged. First, the evaluation relies primarily on benchmark datasets, which may not fully capture real-world complexity or large-scale deployment scenarios.",
    ...
    for entry in large_input:
        small_input.append(entry)
    
    
    small_input = ["Some rep rep rep rep string string is here to be the stringest string there is", "totally a string here"]
    token_to_id, id_to_token, vocab_size, longest_length = build_char_vocab(small_input)
    ...
    inputs, targets = create_sequences(small_input, token_to_id, longest_length, seq_len)
    ...
    for epoch in range(num_epochs):
        ...
        prompt_text = "the "
        prompt_tokens = encode_text(prompt_text, token_to_id, longest_length)
        generated_tokens = model.generate(prompt_tokens, id_to_token, max_length=30)
