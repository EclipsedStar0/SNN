...
class LanguageModel(nn.Module):
    ...
    @staticmethod
    def load_checkpoint(path: str, path_suffix: str, device: str = 'cpu', override_pre_path='') -> Tuple['LanguageModel', Tokenizer]:
        """Load model from checkpoint."""
        with torch.serialization.safe_globals([ModelConfig]):
            checkpoint = torch.load(f"{override_pre_path}models/{path}/{path+path_suffix}"+'.pth', map_location=device, weights_only=False)
        model = LanguageModel(checkpoint['config'])
        model.load_state_dict(checkpoint['model_state_dict'])
        tokenizer = Tokenizer.from_file(f"{override_pre_path}models/{path}/{path}_tokenizer{path_suffix}.json")
...
class TextDataset(Dataset):
    ...
    def __init__(self, texts, seq_len, tokenizer, stride_len, savethis:bool=False, loading_prev:bool=False, prior_name:str = "unnamed_model", moddable_suffix:str='training'):
        ...
        if not loading_prev:
            ...
            for i in tqdm(range(0, len(chunks), BATCH), desc="Tokenizing chunks"):
                batch = chunks[i : i + BATCH]
                encodings = self.tokenizer.encode_batch(batch)
                ids_batch = [enc.ids for enc in encodings]
                chunk_token_ids.extend(ids_batch)
                size_of_base_text += sum(len(ids) for ids in ids_batch)
            ...
            if self.save_this:
                ...
                _flat    = np.array([token for document in chunk_token_ids for token in document], dtype=np.uint16)
                _lengths = np.array([len(document) for document in chunk_token_ids],  dtype=np.uint32)
        ...
        if loading_prev:
            ...
            chunk_token_ids = np.split(_flat, np.cumsum(_lengths[:-1]))
            del _flat, _lengths
            
        print(self.seq_len)
        print(self.stride_len)
        five_perc = self.seq_len / 20 + 3
        
        for token_ids in tqdm(chunk_token_ids, desc="Creating sequences"):
            token_len = len(token_ids)
            if token_len >= self.seq_len:
                # Create overlapping sequences for better data efficiency
                for i in range(0, token_len - self.seq_len, self.stride_len):
                    sequence = token_ids[i:i + self.seq_len]
                    sequence_len_temp = len(sequence)
                    
                    if sequence_len_temp == self.seq_len:
                        #print("A",sequence_len_temp)
                        self.sequences.append(sequence)
                    elif sequence_len_temp < self.seq_len and sequence_len_temp > five_perc:
                        sequence = np.pad(token_ids, (0, self.seq_len-len(token_ids)), mode='constant', constant_values=(self.pad_id, self.pad_id))
                        self.sequences.append(sequence)
                        #print("B",len(sequence))
                        
            elif token_len > five_perc:
                # Pad short sequences
                sequence = np.pad(token_ids, (0, self.seq_len-token_len), mode='constant', constant_values=(self.pad_id, self.pad_id))
...
class OptimizedTrainer:
    ...
    def __init__(self, model: LanguageModel, tokenizer: Tokenizer, config: ModelConfig,
        ...
        num_param = count_parameters(model, no_print=True)
        if num_param > 3000000:
            print("\tDetected Parameter Count exceeds 3M, assuming non-test model and refusing AdamW")
            self.optimizer = torch.optim.AdamW(
                model.parameters(),
                lr=learning_rate,
                weight_decay=weight_decay,
...
""" + "}"
        ...
        training_dataset = TextDataset(train_data, self.config.max_seq_len, self.tokenizer, stride_len, clean_slate_save, loading_prev, prior_name, 'training')
        ...
        torch_generator.manual_seed(GLOBAL_SEED)
             
        torch_generator_test = torch.Generator()
        torch_generator_test.manual_seed(GLOBAL_SEED + 1)
        ...
        #test_b_size = batch_size * gradient_accumulation_steps
        #temp_grad_calc = test_b_size
        #if test_b_size > len(training_dataset.sequences):
        #    # This should only ever happen if using a tiny test model, as otherwise training data will vastly outnumber validation
        #    while len(training_dataset.sequences)/temp_grad_calc < test_b_size:
        #        test_b_size /= 8
        #else:
        #    test_b_size = int(len(training_dataset.sequences) * 0.0002)
        # test_b_size = min(test_b_size, 1000)
        # test_b_size = int(test_b_size)
        
        
        self.max_val_batches = 1000
        testing_dataloader = DataLoader(
            ...
            prefetch_factor=8,
            worker_init_fn=worker_init_fn,
            generator=torch_generator_test
        )
        ...
        def lr_lambda(step):
            ...
            return 0.00001 + 0.9 * 0.5 * (1 + math.cos(math.pi * progress))
    ...
    def _calculate_validation_loss(self, validation_dataloader):
        ...
        with torch.no_grad():
            for data_input, data_output, mask in validation_dataloader:
                ...
                if num_batches >= self.max_val_batches:
                    break
    ...
    #### DEEPSEEK Code
    def compute_gradient_norm(self, dataloader, num_batches=1):
        """
        Compute the L2 norm of the gradient of the average loss (without weight decay)
        w.r.t. all trainable parameters, using the average loss over multiple batches.
        
        This computes: ||∇(1/B Σ L_b)||₂ instead of (1/B) Σ ||∇L_b||₂
        """
        self.model.train()
        
        with torch.enable_grad():
            pbar = tqdm(dataloader, desc="Processing Grad Norm...")
            ...
            for i, (data_input, data_output, mask) in enumerate(pbar):
                if i >= num_batches:
                    break
                ...
                with torch.amp.autocast('cuda', enabled=self.use_amp):
                    ...
                    # Track the actual loss before scaling
                    total_loss += loss.detach().item()
                    
                    # Scale loss for gradient accumulation
                    scaled_loss = loss / num_batches
                
                # Accumulate gradients with scaled loss
                scaled_loss.backward()
                batches_used += 1
            
            # Compute gradient norm from accumulated gradients
            grad_norm_sq = 0.0
            for param in self.model.parameters():
                if param.grad is not None:
                    # Use float() to ensure consistent precision
                    grad_norm_sq += param.grad.data.float().norm(2).item() ** 2
        ...
        return grad_norm, avg_loss
    #### DEEPSEEK Code


def count_parameters(model: nn.Module, no_print=False) -> int:
...
def remove_commented_lines(init_array):
    split_line = init_array.split('\n')
    reforged_data = []
    for line in split_line:
        if not line.startswith('#'):
            reforged_data.append(line)
    return reforged_data
    
def get_evt_data(init_path):
    container_names = get_folder_names(init_path)
    temp_arr = []
    for container in container_names:
        event_file_names = get_file_names(f'{init_path}/{container}/events')
        for event_file_name in event_file_names:
            re_data = try_diff_encodings(f'{init_path}/{container}/events/{event_file_name}').lower()
            temp_arr.append("\n".join(remove_commented_lines(re_data)))
            
    print(f'Num {init_path} Files: {len(temp_arr)}')
    return temp_arr
...
if __name__ == "__main__":
    ...
    if operation_selection == "clean":
        ...
        event_arr = get_evt_data('unprocessed_events')
        test_event_arr = get_evt_data('validation_events')
        ...
        TOKENIZER = train_tokenizer(max_vocab_len, SPECIAL_TOKENS, UNKNOWN_TOKEN, load_preset_toks(), event_arr)
        for index in range(len(event_arr)):
            event_arr[index] = BEGIN_OF_STREAM_TOKEN + event_arr[index] + END_OF_STREAM_TOKEN
        for index in range(len(test_event_arr)):
            test_event_arr[index] = BEGIN_OF_STREAM_TOKEN + test_event_arr[index] + END_OF_STREAM_TOKEN
        ...
        if num_param > 3000000:
            print("\tDetected Parameter Count exceeds 3M, assuming non-test model and disabling torch determinism")
        ...
        trainer = OptimizedTrainer(
            ...
            cycle_length=1000,
            weight_decay=wei_decay,
            gradient_checkpointing=True,  # Set to True if running out of memory
            compile_model=True  # Enable torch.compile for speed
        )   
        ...
        while wants_to_continue:
            mode_of_op = prompt_select('Select Operation', choices=[
                Choice(value='general_inference', title='Manual Inference'),
                Choice(value='analysis', title='Analysis (Grad Norm)'),
                Choice(value='exit', title='Exit')
            ])
            try:
                if mode_of_op == 'exit':
                    wants_to_continue = False
                elif mode_of_op == 'general_inference':
                    usr_prompt = input('Prompt: ')
                    encoded = tokenizer.encode(usr_prompt)
                    ...
                    print(f'\nPrompt: "{usr_prompt}"')
                    print(f'Generated: {text}')
                elif mode_of_op == 'analysis':
                    test_data = get_evt_data('validation_events')
                    trainer = OptimizedTrainer(model, tokenizer, config, learning_rate=0.0, weight_decay=0.001)
                    testing_dataset = TextDataset(test_data, config.max_seq_len, tokenizer, 256, False, False, '', 'testing')
                    ...
                    grad_norm = trainer.compute_gradient_norm(dataloader, num_batches=400)
                    print(grad_norm)
            except KeyboardInterrupt:
                pass
            except Exception as e:
