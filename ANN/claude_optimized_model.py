...
class OptimizedTrainer:
    ...
    def __init__(self, model: LanguageModel, tokenizer: Tokenizer, config: ModelConfig,
        ...
        # Loss function
        self.criterion = nn.CrossEntropyLoss(ignore_index=tokenizer.token_to_id(PAD_TOKEN), label_smoothing=self.config.label_smoothing)
...
""" + "}"
        ...
        self.max_val_batches = max(test_b_size, 1000)
    ...
    def _generate_sample(self):
        ...
        prompts = [
            ...
            #f"{BOS_TOKEN}From the highest peaks to the lowest",
            f"There was once a time when the King",
            #"Consider",
            #"Lemme",
            #"Narrate:",
            #"Directive:",
...
if __name__ == "__main__":
    ...
    And then, at the southern pole of the Earth, is Antarctica, the Earth's southernmost continent, it has the smallest population of all seven continents despite being 40% larger than Europe with a population numbering at several thousand during the summer wonths-- a number that plummets to a mere thousand come winter. On the other end of the globe, on the Earth's north pole, is the Arctic Circle a frozen collection of seas that upon which the northern tips of North America, Europe, and Asia converge upon.{EOS_TOKEN}""",
        ...
        print("Fixing text encoding...")
        max_workers = os.cpu_count()  # or pin to e.g. 8

        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            training_data = list(tqdm(
                executor.map(ftfy.fix_text, training_data, chunksize=64),
                total=len(training_data),
                desc="Fixing file data"
            ))
        ...
        #    training_data
        #)
        tokenizer = Tokenizer.from_file(f"V:/CursorLocker/SNN/ANN/models/T_Test23_12L_512Dim_16KVk_64Overlap_BetaSettings/T_Test23_12L_512Dim_16KVk_64Overlap_BetaSettings_tokenizer_CleanSlate.json")
