...
class OptimizedTrainer:
    ...
    def __init__(self, model: LanguageModel, tokenizer: Tokenizer, config: ModelConfig, qat_cfg : QATConfig,
...
\t"Bits" = {self.qat_cfg.bits},
\t"quantize_activations" = {self.qat_cfg.quantize_activations},
\t"act_quant_per_tensor" = {self.qat_cfg.act_quant_per_tensor},
\t"quantize_weights" = {self.qat_cfg.quantize_weights},
\t"weight_quant_per_channel" = {self.qat_cfg.weight_quant_per_channel},
\t"qat_start_step" = {self.qat_cfg.qat_start_step},
\t"ema_momentum" = {self.qat_cfg.ema_momentum},
\t"skip_modules" = {self.qat_cfg.skip_modules}
""" + "}"
        ...
        for epoch in range(epochs-current_epoch):
            ...
            for batch_idx, (data_input, data_output, mask) in enumerate(pbar):
                ...
                if (batch_idx + 1) % gradient_accumulation_steps == 0:
                    maybe_enable_qat(self.model, self.qat_cfg, global_step)
                    if self.use_amp:
...
if __name__ == "__main__":
    ...
    qat_cfg = QATConfig(
        bits=8,                  # INT8 (use 4 for INT4)
        quantize_weights=True,
        quantize_activations=True,
        weight_quant_per_channel=True,
        act_quant_per_tensor=True,
        qat_start_step=150,     # float-only warm-up for 2000 steps, then QAT
        ema_momentum=0.1,
        skip_modules=["lm_head"],  # keep the output head in float32
    )
    ...
    And then, at the southern pole of the Earth, is Antarctica, the Earth's southernmost continent, it has the smallest population of all seven continents despite being 40% larger than Europe with a population numbering at several thousand during the summer wonths-- a number that plummets to a mere thousand come winter. On the other end of the globe, on the Earth's north pole, is the Arctic Circle a frozen collection of seas that upon which the northern tips of North America, Europe, and Asia converge upon.{EOS_TOKEN}""",
        ...
        model = LanguageModel(config).to(device)
        model = wrap_model_for_qat(model, qat_cfg)
