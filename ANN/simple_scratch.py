        ...
        for epoch in range(epochs):
            ...
            if epoch % 5 == 0:
                generated_text = generator.generate(
                    ...
                    prompt="Project Eidolon proposed the acquisition of several squadrons of mining-drones",
                ...
                generated_text = generator.generate(
                    max_tokens_to_generate=60,
                    prompt="Méfait drew their holstered laser pistol and silently crouched towards the wall opposite of Trèfle",
                    padding_token=tokenizer.character_to_token('<PAD>')
                )
                generated_text = generated_text.replace('<PAD>', '')
                print(generated_text)
                generated_text = generator.generate(
                    max_tokens_to_generate=60,
                    prompt="*Nodding almost with anticipation, Rowle answers.* \"Bring Petrov in under-guard and continue the sweeps",
                    padding_token=tokenizer.character_to_token('<PAD>')
                )
                generated_text = generated_text.replace('<PAD>', '')
                print(generated_text)
                model.save_checkpoint(f"models/text_completion_17_{epoch}_of_{epochs}.pth")
...
# Train the model
optimizer = torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=1e-4, betas=(0.9, 0.95))
trainer = Trainer(model, tokenizer, optimizer)
trainer.train(sequences, epochs=100, batch_size=512)
...
model.save_checkpoint("models/text_completion_17.pth")