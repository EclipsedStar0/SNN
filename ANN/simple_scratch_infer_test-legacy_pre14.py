...
class Tokenizer:
    ...
    def tokenize(self, text):
        ...
        for character in text:
            if character not in self.dictionary:
                tokenized.append(self.dictionary["<unk>"])
            else:
                tokenized.append(self.dictionary.get(character))
...
class DecoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.layer_normalization_1 = torch.nn.LayerNorm(embedding_dimension).to(device)
        self.layer_normalization_2 = torch.nn.LayerNorm(embedding_dimension).to(device)
...
class LanguageModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.layer_normalization = torch.nn.LayerNorm(embedding_dimension).to(device)
        ...
        self.lm_head = LMHead(embedding_dimension, number_of_tokens).to(device)

    def forward(self, x, mask):
        # Compute the token embeddings
        # token_embeddings dimensions are: (batch_size, sequence_length, embedding_dimension)
        token_embeddings = self.token_embedding(x).to(device)

        # Compute the positional encoding
        # positional_encoding dimensions are: (batch_size, sequence_length, embedding_dimension)
        positional_encoding = self.positional_encoding(token_embeddings).to(device)

        # Post embedding layer normalization
        positional_encoding_normalized = self.layer_normalization(positional_encoding).to(device)

        decoder_outputs = self.decoder(positional_encoding_normalized, mask).to(device)
        lm_head_outputs = self.lm_head(decoder_outputs).to(device)
    ...
    def train(self, data: list[str], epochs, batch_size):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(data), batch_size):
                sequence_tensor = torch.tensor(data[i: i + batch_size], dtype=torch.long).to(device)

                # Create the mask tensor for the batch, where 1 means the token is not a padding token
                mask_tensor = torch.ones_like(sequence_tensor).to(device)
            ...
            for batch in batches:
                ...
                input_tensor = torch.zeros((batch_size, self.model.max_sequence_length + 1), dtype=torch.long).to(device)
                mask_tensor = torch.zeros((batch_size, self.model.max_sequence_length + 1), dtype=torch.long).to(device)
                ...
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), 0.5).to(device)
            ...
            print('Epoch:', epoch, 'Time:', f'{(time.time()-startTime):.5}','seconds;', 'Loss:', epoch_loss)
...
model = model.load_checkpoint("models/text_completion_13.pth")
model.eval()
max_tokens_to_generate = 60
generator = Generator(model, tokenizer)

promptStr = "there exists only"
